...
local function GetJobs()
    local p = promise.new()
    QBCore.Functions.TriggerCallback('RL-multijob:getJobs', function(result)
        p:resolve(result)
    end)
...
RegisterNUICallback('selectjob', function(data, cb)
    TriggerServerEvent("RL-multijob:changeJob", data["name"], data["grade"])
    local onDuty = false
...
RegisterNUICallback('removejob', function(data, cb)
    TriggerServerEvent("RL-multijob:removeJob", data["name"], data["grade"])
    local jobs = GetJobs()
    jobs[data["name"]] = nil
    cb(jobs)
end)
